...
execute("INSERT INTO `sale` (`invoice_id`,`customer_id`,`cost_t`,`cost_co`,`num`,`type`,`cost_wasl`,`date`,`discount`,`unit`,`name_product`,`sale_type`,`status`,`note`) VALUES('$invoice_id','$customer_id
...
direct("sale_helka_invoice.php?invoice_id=$invoice_id");
...
<form method="post" action="sale_helka_invoice.php?invoice_id=<?=$invoice_id?>">
    ...
    <label>ناوی کڕیار</label>
            <select name="dealer_id" id="dealer_id1"  class="form-control col-md-10 mx-auto">
                <?php
                 ...
                 <option <?php if($customerId==$dealer['id']) echo 'selected="selected"'; ?> value="<?=$dealer['id']?>"> <?=$dealer['name']?> </option>
                <?php   } ?>
            </select>
        </div> 
...
<table  class="table  table-striped table-bordered  text-center" id="tab_logic" dir="rtl" style="zoom:85%">
       ...
       <tr id="row_id_1" class="text-center">
        ...
        <div class="form-group">
                     ...
                     <select  name="type[]" id="type1" index="<?=$count?>"  class="form-control type col-md-10 mx-auto">
...
if (post('return_buy')) {
    ...
    $loc="sale_helka_invoice.php?invoice_id=".$invoice_id;
    direct($loc);
  }
